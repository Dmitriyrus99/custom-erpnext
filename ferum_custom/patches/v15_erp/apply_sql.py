...
def execute():
    ...
    db_type = getattr(getattr(frappe, 'db', None), 'db_type', None) or getattr(frappe.conf, 'db_type', None)
    if str(db_type).lower() not in {"postgres", "postgresql"}:
        ...
        try:
            ...
            frappe.logger().info("ferum_custom.v15_erp.apply_sql: skipping SQL patches on non-Postgres db_type=%s", db_type)
        except Exception:
            pass
        return
