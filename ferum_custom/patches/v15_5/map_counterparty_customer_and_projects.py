...
def execute():
    ...
    for cp in frappe.get_all(
        ...
        if candidate:
            frappe.db.set_value(
                "Counterparty", cp.name, "customer", candidate, update_modified=False
            )
    ...
    for sp in frappe.get_all(
        ...
        for key in (sp.name, sp.project_name):
            ...
            project = frappe.db.get_value(
                "Project",
                (
                    {"project_name": key, "company": sp.company}
                    if sp.company
                    else {"project_name": key}
                ),
        ...
        if project:
            frappe.db.set_value(
                "Service Project", sp.name, "erp_project", project, update_modified=False
            )
