...
from ferum_custom.patches.utils_migration import _log, find_or_create_project, migrate_attachments, has_doctypes
...
def execute():
    ...
    for name in names:
        try:
            ...
            if inv.counterparty_type == "Customer":
                ...
                if getattr(inv, "sales_invoice", None) and frappe.db.exists("Sales Invoice", inv.sales_invoice):
    ...
    _log(f"migrate_invoice_to_erpnext: ok={ok} skipped={skipped} attachments_ok={att_ok} attachments_skipped={att_skip}")
