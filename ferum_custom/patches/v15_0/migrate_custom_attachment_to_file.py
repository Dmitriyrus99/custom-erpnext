...
def execute():
    ...
    """
    ok = skipped = 0
    rows = frappe.get_all("Custom Attachment", fields=["name", "file_url", "file_name", "linked_doctype", "linked_docname"])
    for r in rows:
        try:
            ...
            skipped += 1
            frappe.log_error(frappe.get_traceback(), f"CustomAttachment to File migration failed: {r.name}")
