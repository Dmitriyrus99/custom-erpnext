...
def _parse_service_object_from_text(text: str | None) -> str | None:
    if not text:
        return None
...
def _service_object_default_engineer_from_issue(doc) -> str | None:
    ...
    try:
        if doc.subject and doc.subject.startswith("Scheduled Maintenance:"):
            ...
            if frappe.db.exists("Service Object", so_name):
                return frappe.db.get_value("Service Object", so_name, "default_engineer")
            obj = frappe.db.get_value("Service Object", {"object_name": so_name}, "name")
            if obj:
                return frappe.db.get_value("Service Object", obj, "default_engineer")
    except Exception:
        pass
    ...
    try:
        ...
        if so:
            if frappe.db.exists("Service Object", so):
                return frappe.db.get_value("Service Object", so, "default_engineer")
            obj = frappe.db.get_value("Service Object", {"object_name": so}, "name")
            if obj:
                return frappe.db.get_value("Service Object", obj, "default_engineer")
    except Exception:
        pass
...
def before_insert(doc, method=None):  # called via hooks
    ...
    try:
        ...
        eng = _service_object_default_engineer_from_issue(doc)
        if eng:
            doc.assigned_engineer = eng
            return
    except Exception:
