...
class TestServiceAPI(FrappeTestCase):
    ...
    def setUpClass(cls):
        ...
        if not frappe.db.exists("User Permission", {
            "user": "test_client@example.com",
            "allow": "Customer",
            "for_value": cls.customer
        }):
    ...
    def test_confirm_issue_completion_api(self):
        ...
        comments = frappe.get_all("Comment", filters={"reference_doctype": "Issue", "reference_name": issue_name, "comment_type": "Comment"})
        self.assertTrue(len(comments) > 0)
        self.assertTrue("Client confirmed completion" in frappe.get_doc("Comment", comments[0]["name"]).content)
        frappe.set_user("Administrator")
    ...
    def test_confirm_timesheet_report_api(self):
        ...
        # Verify a comment was added
        comments = frappe.get_all("Comment", filters={"reference_doctype": "Timesheet", "reference_name": timesheet_name, "comment_type": "Comment"})
        self.assertTrue(len(comments) > 0)
        self.assertTrue("Client confirmed Timesheet Report" in frappe.get_doc("Comment", comments[0]["name"]).content)
        frappe.set_user("Administrator")
