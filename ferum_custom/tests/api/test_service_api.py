...
class TestServiceAPI(FrappeTestCase):
    ...
    def setUpClass(cls):
        ...
        cls.asset = smoke_tools.ensure_asset(
            "Test Asset 1", customer=cls.customer, company=cls.company
        )
        ...
        if not frappe.db.exists("Role", "Client"):
            frappe.get_doc({"doctype": "Role", "role_name": "Client"}).insert(
    ...
    def test_confirm_timesheet_report_api(self):
        ...
        self.assertTrue(len(comments) > 0)
        self.assertTrue(
            "Client confirmed Timesheet Report"
            in frappe.get_doc("Comment", comments[0]["name"]).content
        )
        frappe.set_user("Administrator")
