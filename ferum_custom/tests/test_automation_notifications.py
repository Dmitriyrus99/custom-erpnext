...
def test_get_report_recipients_handles_multiple_roles(monkeypatch):
    ...
    monkeypatch.setattr(
        automation.frappe, "get_users_with_role", fake_get_users_with_role, raising=False
    )
...
def test_run_permission_audit_queries_combined_doctype(monkeypatch):
    ...
    monkeypatch.setattr(automation.frappe, "get_all", fake_get_all)
    monkeypatch.setattr(
        "ferum_custom.ferum_custom.automation.get_report_recipients",
        lambda roles: ["admin@example.com"],
    )
    monkeypatch.setattr(
