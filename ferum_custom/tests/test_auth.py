...
def test_login_requires_otp_when_two_factor_enabled(monkeypatch, dummy_login_manager):
	...
	monkeypatch.setattr(
		auth, "pyotp", types.SimpleNamespace(TOTP=lambda secret: types.SimpleNamespace(verify=lambda _: True))
	)
	monkeypatch.setattr(
...
def test_login_with_valid_otp(monkeypatch, dummy_login_manager):
	...
	monkeypatch.setattr(
		auth,
		"jwt",
		types.SimpleNamespace(encode=lambda payload, secret, algorithm=None: f"jwt::{payload['sub']}"),
	)
	monkeypatch.setattr(auth, "get_setting", lambda key: "shared-secret" if key == "jwt_secret" else None)
