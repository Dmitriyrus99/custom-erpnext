...
class TestClientCustomerPermissions(FrappeTestCase):
    def setUp(self):
        ...
        for customer in ("Cust A", "Cust B"):
            ...
            self.customers[customer] = frappe.db.get_value(
                "Customer", {"customer_name": customer}, "name"
            )
        for obj_name, customer in (("Obj-A", "Cust A"), ("Obj-B", "Cust B")):
            smoke_tools.ensure_asset(
                obj_name, customer=self.customers[customer], company=self.company
            )
        ...
        if not frappe.db.exists(
            "User Permission",
            {
                "user": "cust1@example.com",
                "allow": "Customer",
                "for_value": self.customers["Cust A"],
            },
        ):
            up = frappe.new_doc("User Permission")
            up.user = "cust1@example.com"
    ...
    def test_client_sees_requests_by_customer(self):
        ...
        for title, so_obj in (("SR-A", "Obj-A"), ("SR-B", "Obj-B")):
            ...
            issue_doc.customer = (
                self.customers["Cust A"] if so_obj == "Obj-A" else self.customers["Cust B"]
            )
