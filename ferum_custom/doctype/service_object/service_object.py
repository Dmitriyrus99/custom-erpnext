    ...
    def on_trash(self):
        ...
        if parents:
            ...
            if active_projects:
                frappe.throw(
                    _(
                        "Cannot delete Service Object. Linked to active Service Projects: {projects}"
                    ).format(projects=", ".join(active_projects))
                )
        ...
        if active_requests:
            frappe.throw(
                _(
                    "Cannot delete Service Object. Linked to active Service Requests: {requests}"
                ).format(requests=", ".join(active_requests))
            )
