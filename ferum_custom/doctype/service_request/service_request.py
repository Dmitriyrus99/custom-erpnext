...
from ferum_custom.ferum_custom.utils import user_roles
...
class ServiceRequest(Document):
            ...
            if not getattr(self, "assigned_to", None):
                ...
                if getattr(self, "service_object", None):
                    eng = frappe.db.get_value(
                        "Service Object", self.service_object, "default_engineer"
                    )
                    if eng:
    ...
    def set_customer_and_project(self):
        ...
        needs_sync = (
            self.is_new()
            or self.has_value_changed("service_object")
        ...
        if self.service_object:
            ...
            dept, company, customer, default_eng = frappe.db.get_value(
                "Service Project",
                self.project,
                ["service_department", "company", "customer", "default_engineer"],
            )
