...
def create_payment_entry_from_payment(payment_name: str) -> str:
    ...
    if pay.direction == "out":
        frappe.throw(
            _("Payment {0}: supplier payouts not supported in automation yet").format(payment_name)
        )
    ...
    if not customer:
        frappe.throw(
            _("Payment {0}: customer is required to create Payment Entry").format(payment_name)
        )
