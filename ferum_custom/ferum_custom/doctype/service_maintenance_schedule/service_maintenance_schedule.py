                ...
                try:
                    ...
                    with contextlib.suppress(Exception):
                        if getattr(item, "service_object", None):
                            engineer = frappe.db.get_value("Service Object", item.service_object, "default_engineer")
                        if not engineer and getattr(schedule, "service_project", None):
                            engineer = frappe.db.get_value("Service Project", schedule.service_project, "default_engineer")
                    if engineer:
                        try:
                            from frappe.desk.form.assign_to import add as add_assignment  # type: ignore[import-not-found]
