...
class FerumCustomSettings(Document):
	...
	def _validate_integration_settings(self):
		if self.enable_google_drive_sync and not self.google_drive_root_folder_id:
			frappe.throw(
				_("Drive Root Folder ID is required when Google Drive sync is enabled.")
			)
		if self.enable_google_sheets_sync and not self.google_service_account_json:
			frappe.throw(
				_("Google Service Account JSON is required to sync invoices to Sheets.")
			)
		if self.enable_telegram_notifications and not self.telegram_bot_token:
			frappe.throw(_("Telegram bot token must be supplied when Telegram notifications are enabled."))
	...
	def _ensure_service_account_file_private(self):
		...
		try:
			...
			frappe.log_error(frappe.get_traceback(), "Failed to enforce privacy on Google service account JSON")
