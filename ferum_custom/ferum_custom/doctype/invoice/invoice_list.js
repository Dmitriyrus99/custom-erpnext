frappe.listview_settings["Invoice"] = {
    onload(listview) {
        ...
        listview.page.add_inner_button(__("Customer"), () => {
            listview.filter_area.clear();
            listview.filter_area.add([["Invoice", "counterparty_type", "=", "Customer"]]);
            listview.refresh();
        }, __("Type"));

        listview.page.add_inner_button(__("Subcontractor"), () => {
            listview.filter_area.clear();
        ...
        if (can_mark_sent) listview.page.add_actions_menu_item(__("Mark as Sent"), async () => {
            const selected = listview.get_checked_items();
            if (!selected.length) {
                frappe.msgprint({ message: __("Select at least one invoice."), indicator: "orange" });
                return;
            }
        ...
        const can_mark_paid = ["Chief Accountant", "System Manager"].some((r) => frappe.user.has_role(r));
        if (can_mark_paid) listview.page.add_actions_menu_item(__("Mark as Paid"), async () => {
            const selected = listview.get_checked_items();
            if (!selected.length) {
                frappe.msgprint({ message: __("Select at least one invoice."), indicator: "orange" });
                return;
