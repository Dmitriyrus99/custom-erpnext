...
def generate_from_template(template_name: str, attach_to_doctype: str, attach_to_name: str) -> str:
	...
	ctx = (
		...
		else _ctx_for_task(attach_to_name)
		if attach_to_doctype == "Task"
		else {"doc": frappe.get_doc(attach_to_doctype, attach_to_name)}
	)

	html = _render_template(tpl.template_body, ctx)
	content: bytes
	file_name = frappe.render_template(
		tpl.file_name_pattern or "{{ doc.name }}.pdf" if tpl.output == "PDF" else "{{ doc.name }}.html", ctx
	)
	if tpl.output == "PDF":
...
def on_task_update(doc, method: str | None = None) -> None:
	try:
		changed = bool(doc.has_value_changed and doc.has_value_changed("status"))
	except Exception:
		changed = False
