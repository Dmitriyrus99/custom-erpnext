...
def generate_issues_from_schedules() -> dict:
    """Generate ERPNext Issues for all due Service Maintenance Schedules.
    ...
    for row in schedules:
        try:
            ...
            for item in schedule.items:
                try:
                    subj = (
                        f"Scheduled Maintenance: {item.service_object} ({schedule.schedule_name})"
                    )
                    desc = (
                        item.description
                    ...
                    issue = frappe.get_doc(
                        {
                            ...
                            "project": std_project,
                            # Link back to schedule + affected object for reporting
                            "service_maintenance_schedule": schedule.name,
                    ...
                    try:
                        ...
                        log_event(
                            ...
                            ref_doctype="Issue",
                            ref_docname=issue.name,
                            message=f"Created from schedule {schedule.name}",
                            details={
