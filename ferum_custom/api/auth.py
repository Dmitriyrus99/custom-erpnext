...
from frappe.twofactor import get_otpsecret_for_, get_verification_method, two_factor_is_enabled
...
def jwt_before_request() -> None:
    ...
    try:
        ...
        try:
            form_cmd = (
                getattr(getattr(frappe.local, "form_dict", object()), "cmd", None) or ""
            ).strip()
        except Exception:
            form_cmd = None
        in_namespace = False
