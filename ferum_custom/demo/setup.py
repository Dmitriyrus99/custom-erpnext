...
def _get_or_create_maintenance_schedule(customer: str, project: str, service_object: str) -> str:
    key = f"MS-{project}"
    exists = frappe.db.exists("Maintenance Schedule", {"schedule_name": key})
    if exists:
        return exists
...
def create_demo_data() -> dict:
    ...
    if not any(so.service_object == service_object for so in frappe.get_doc("Service Project", project).objects):
