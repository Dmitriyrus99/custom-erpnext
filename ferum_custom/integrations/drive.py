...
def _ensure_folder(drive, name: str, parent_id: str | None) -> str | None:
    query = (
        "mimeType='application/vnd.google-apps.folder' and name='" + name.replace("'", "\\'") + "'"
    )
    if parent_id:
    ...
    try:
        ...
        if files:
            return files[0]["id"]
        body: dict[str, object] = {"name": name, "mimeType": "application/vnd.google-apps.folder"}
        if parent_id:
...
def upload_bytes(
    ...
    for attempt in range(1, max_attempts + 1):
        try:
            ...
            frappe.log_error(
                frappe.get_traceback(), f"Drive upload attempt {attempt} failed: {context}"
            )
    ...
    if last_error:
        ...
        try:
            ...
            metrics_inc(
                "ferum_integration_drive_upload_total", {"result": "error", "category": category}
            )
        except Exception:
            pass
    return None
...
def delete_file(file_id: str) -> bool:
    ...
    try:
        ...
        try:
            metrics_inc(
                "ferum_integration_drive_delete_total", {"result": "error", "category": category}
            )
        except Exception:
            pass
        return False
...
def healthcheck() -> dict[str, Any]:
    ...
    if build is None or MediaInMemoryUpload is None:
        try:
            metrics_inc(
                "ferum_integration_drive_health_total",
    ...
    if not root:
        try:
            metrics_inc(
                "ferum_integration_drive_health_total", {"result": "error", "reason": "no_root"}
            )
        except Exception:
            pass
    ...
    if not service:
        try:
            metrics_inc(
                "ferum_integration_drive_health_total", {"result": "error", "reason": "init_failed"}
            )
        except Exception:
            pass
