...
- [ ] Контрагент: добавить ссылку `customer` в `Counterparty`, мигрировать ссылки в Payment/Contract/Projects на `Customer`; определить путь отказа от дублирования. *(в работе: Invoice/Payment/Contract уже требуют Customer, миграция Counterparty→Customer выполнена частично)*
