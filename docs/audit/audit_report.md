...
- Риски: присутствуют секреты/учетные данные в репозитории (config/.env.integrations, redis *.acl), бинарные дампы/файлы состояния (config/pids/*.rdb), установочные логи и lock-файлы коммичены в VCS, дублирующиеся и конфликтующие документы (несколько audit/plan/architecture материалов), потенциальный конфликт npm/yarn конфигураций.
...
- .gitignore
  ...
  - Статус: требует ревизии (учесть .env, *.rdb, *.lock, logs, dumps).
...
- scripts/precommit/check_doctype_inits.py
  - Назначение: валидация __init__.py в DocType пакетах.
  - Статус: актуально.
...
- Upstream ERPNext содержит обширные тесты (apps/erpnext/…/test_*.py, test_records.json).
...
| install_*.log | Установочные логи | Устарело | MOVE → legacy | Не хранить логи в git |
...
| config/*/*.lock, *.lock в корне | Lock-файлы | Устарело | Низкий | Удалить/MOVE → legacy, добавить в .gitignore |
...
- Бинарные и runtime-файлы (redis *.rdb, логи install_*.log, lock-файлы) — захламление и риск.
...
- Удалить/исключить из git файлы с секретами и runtime:
  ...
  - config/redis_*.acl, config/pids/*.rdb, * .lock → убрать и добавить в .gitignore.
- Обновить .gitignore:
  - .env, *.acl, *.rdb, *.log, *.lock, /legacy/, /sites/*/private/backups/, node_modules, dist, assets, .bench, .DS_Store и т.п.
