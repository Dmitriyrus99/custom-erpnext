...
def test_drive_healthcheck_success(monkeypatch):
    ...
    monkeypatch.setattr(
        drive, "MediaInMemoryUpload", lambda data, mimetype, resumable=False: ("media", data)
    )
    monkeypatch.setattr(
        drive,
        "get_setting",
        lambda field, default=None: (
            "root-folder" if field == "google_drive_root_folder_id" else None
        ),
    )
