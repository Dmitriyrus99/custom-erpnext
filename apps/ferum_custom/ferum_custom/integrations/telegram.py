...
from ferum_custom.ferum_custom.settings import get_list_setting, get_setting, is_feature_enabled
...
def send_message(text: str, chat_id: str | None = None, max_retries: int = 3) -> bool:
    ...
    if not token or not chat or requests is None:
        try:
            frappe.logger().warning(
                "Telegram not configured or requests missing. Message suppressed."
            )
        except Exception:
            pass
        try:
            metrics_inc(
                "ferum_integration_telegram_send_total",
                {"result": "error", "reason": "not_configured"},
            )
        except Exception:
            pass
        return False
    ...
    if not is_chat_allowed(chat):
        frappe.logger().warning(
            "Telegram chat %s is not in the allowlist; message suppressed.", chat
        )
        try:
            metrics_inc(
                "ferum_integration_telegram_send_total",
                {"result": "error", "reason": "not_allowed"},
            )
        except Exception:
            pass
        return False
