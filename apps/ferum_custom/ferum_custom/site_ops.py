...
from ferum_custom.ferum_custom.security.api_guard import require_post_if_http, require_roles_if_http
...
def apply_ru_workspaces() -> dict:
    ...
    # Hide EN workspaces
    _hide_workspaces(
        ["Chief Accountant", "Office Manager", "Project Manager", "Accounting", "Engineer"]
    )
    ...
    if frappe.db.exists("Workspace", "Руководитель отдела"):
        ...
        _ensure_shortcut(
            ws, label="Нераспределенные заявки", type_="Report", link_to="Unassigned Issues"
        )
        _ensure_shortcut(
            ws,
            label="Открытые заявки по инженерам",
            type_="Report",
            link_to="Open Issues by Engineer",
        )
    ...
    if frappe.db.exists("Workspace", "Управление проектами"):
        ...
        _ensure_shortcut(
            ws, label="Новый счет (субподрядчик)", type_="URL", url="/app/purchase-invoice/new"
        )
        if frappe.db.exists("Report", "Issues by Project"):
            _ensure_shortcut(
                ws, label="Заявки по проектам", type_="Report", link_to="Issues by Project"
            )
    ...
    try:
        ...
        _ensure_shortcut(
            ws,
            label="Графики обслуживания",
            type_="DocType",
            link_to="Service Maintenance Schedule",
        )
        _ensure_shortcut(
            ws, label="Новый график", type_="URL", url="/app/service-maintenance-schedule/new"
        )
        _ensure_shortcut(ws, label="Объекты", type_="DocType", link_to="Asset")
        _ensure_shortcut(ws, label="Заявки", type_="URL", url="/app/issue")
        if frappe.db.exists("Report", "Due Service Maintenance Schedules"):
            _ensure_shortcut(
                ws,
                label="Просроченные графики",
                type_="Report",
                link_to="Due Service Maintenance Schedules",
            )
    except Exception:
        frappe.log_error(
...
def backup_to_drive() -> dict:
    ...
    try:
        ...
        if is_feature_enabled("enable_backup_encryption"):
            passphrase = (
                get_setting("backup_encryption_key") or get_or_generate_backup_encryption_key()
            )
...
def backfill_drive_ids(limit: int | None = 200, dry_run: bool = False) -> dict:
    ...
    try:
        ...
        frappe.log_error(
            frappe.get_traceback(), "backfill_drive_ids: list custom attachments failed"
        )
...
def weekly_overdue_maintenance_schedules_email() -> dict:
    ...
    try:
        ...
        frappe.log_error(
            frappe.get_traceback(), "weekly_overdue_maintenance_schedules_email failed"
        )
    return {"status": "error", "sent": False}
...
@frappe.whitelist()
def cleanup_backups_retention(
    daily_keep: int = 7, weekly_keep: int = 4, monthly_keep: int = 6
) -> dict:
    ...
    try:
        ...
        dumps = sorted(
            [
                p
                for p in backups_dir.iterdir()
                if p.is_file() and p.suffix in (".gz", ".sql", ".gpg")
            ],
...
def audit_fix_tasks() -> dict:
    ...
    sch = frappe.db.get_value(
        "Service Maintenance Schedule", {"schedule_name": ["like", "Smoke %"]}, "name"
    )
    if not sch:
    ...
    if not doc.items:
        doc.append(
            "items", {"service_object": asset_name[0], "description": "Routine check (audit)"}
        )
        doc.save(ignore_permissions=True)
    ...
    if not frappe.db.exists(
        "User Permission", {"user": uname, "allow": "Customer", "for_value": cust}
    ):
...
def cleanup_users(keep_emails: list[str] | None = None, company: str | None = None) -> dict:
    ...
    frappe.db.commit()
    return {"status": "ok", "disabled": disabled, "kept": kept, "granted_company": granted_company}
