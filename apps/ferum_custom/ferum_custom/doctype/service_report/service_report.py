...
class ServiceReport(Document):
                ...
                if self.status == "Approved" and getattr(self, "service_request", None):
                    ...
                    if cust:
                        _notify_clients(
                            ...
                            _("Your service report {0} was approved and is available.").format(
                                self.name
                            ),
                        )
        except Exception:
        ...
        for item in self.documents:
            ...
            try:
                ...
                if not mime:
                    ...
                    m2, _encoding = guess_type(name_or_url)
                    mime = (m2 or "").lower()
        ...
        for item in self.documents or []:
            ...
            try:
                ...
                frappe.log_error(
                    ...
                    f"Failed to sync attachment link for {item.custom_attachment}",
                )
    ...
    def validate_workflow_transitions(self):
        ...
        """
        old_status = (
            frappe.db.get_value("Service Report", self.name, "status")
            if not self.is_new()
            else None
        )
    ...
    def validate_work_items(self):
        if not self.work_items:
            frappe.throw(
                _("At least one Work Item is required before submitting a Service Report.")
            )
...
def has_permission(doc, user: str | None = None) -> bool:
    ...
    if doc.service_request:
        ...
        if info:
            ...
            service_department = service_department or (service_request_info or {}).get(
                "service_department"
            )
