...
try:
    ...
    from gspread.exceptions import (
        CellNotFound as GSpreadCellNotFound,  # type: ignore[import-untyped]
    )
except Exception:  # pragma: no cover
    gspread = None  # Optional dependency; handled at runtime

    class _FallbackCellNotFound(Exception):  # type: ignore[valid-type]
        pass

    GSpreadCellNotFound = _FallbackCellNotFound
...
def get_google_sheet():
    ...
    try:
        ...
        frappe.log_error(
            f"Google Sheets connection failed: {e!s}", "Google Sheets Connection Error"
        )
        try:
    ...
    def validate(self):
        ...
        try:
            ...
            if getattr(self, "service_report", None) and not getattr(self, "service_request", None):
                sr_request = frappe.db.get_value(
                    "Service Report", self.service_report, "service_request"
                )
                if sr_request:
            ...
            if getattr(self, "customer", None):
                ...
                if not getattr(self, "counterparty_name", None):
                    self.counterparty_name = frappe.db.get_value(
...
def sync_to_google_sheets(docname: str):
    ...
    try:
        ...
        try:
            if getattr(doc, "project", None):
                ...
                if isinstance(pm_email, dict):
                    pm_email = pm_email.get("project_manager.email") or pm_email.get(
                        "project_manager"
                    )
        except Exception:
            pm_email = None
        ...
        category, context = _classify_failure(e)
        _notify_admins(
            "Sheets sync failed",
            f"Failed to sync invoice {doc.name} to Google Sheet. Error: {context}",
        )
        try:
            metrics_inc(
                "ferum_integration_sheets_sync_total", {"result": "error", "category": category}
            )
        except Exception:
            pass
...
def _classify_failure(exc: Exception) -> tuple[str, str]:
    """Classify common integration failures for metrics and notifications."""
    msg = str(exc) or exc.__class__.__name__
    msg_lower = msg.lower()

    if "permission" in msg_lower or "forbidden" in msg_lower or "unauthorized" in msg_lower:
        return ("auth", msg)
    if "quota" in msg_lower or "rate limit" in msg_lower or "too many requests" in msg_lower:
        return ("quota", msg)
