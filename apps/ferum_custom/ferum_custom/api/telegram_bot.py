...
def _cmd_ping(ctx: TelegramContext) -> None:
    ...
    if status == "ok":
        ...
        ctx.reply(
            _("Telegram OK (bot: {0})").format(
                details.get("username") or details.get("bot_id") or "-"
            )
        )
    else:
...
def _dispatch_command(ctx: TelegramContext) -> None:
    ...
    if handler is None:
        try:
            metrics_inc(
                "ferum_integration_telegram_command_total",
                {"command": "unknown", "result": "blocked"},
            )
        except Exception:
            pass
    ...
    if ctx.command in ADMIN_COMMANDS:
        if not telegram_integration.is_admin(ctx.user):
            try:
                metrics_inc(
                    "ferum_integration_telegram_command_total",
                    {"command": ctx.command, "result": "blocked"},
                )
            except Exception:
                pass
        ...
        if not _has_admin_role(ctx):
            ...
            ctx.reply(
                _(
                    "Admin mapping required. Ask admin to link your Telegram to an ERP user with System Manager or Telegram Admin role."
                )
            )
            return
    ...
    try:
        metrics_inc(
            "ferum_integration_telegram_command_total",
            {"command": ctx.command, "result": "success"},
        )
    except Exception:
        pass
...
def _verify_secret() -> None:
    ...
    try:
        header_secret = (
            frappe.request.headers.get("X-Telegram-Bot-Api-Secret-Token") or ""
        ).strip()
    except Exception:
        header_secret = None
...
def _verify_ip_allowlist() -> None:
    ...
    allowlist = (
        frappe.local.conf.get("telegram_ip_allowlist") if hasattr(frappe.local, "conf") else None
    ) or os.environ.get("TELEGRAM_IP_ALLOWLIST", "")
...
def handle_update(
    secret: str | None = None, update: str | dict[str, Any] | None = None
) -> dict[str, Any]:
    ...
    try:
        ...
        if not target_user and ctx.chat_id:
            u = frappe.get_all(
                "User", filters={"telegram_chat_id": str(ctx.chat_id)}, pluck="name", limit=1
            )
            if u:
                target_user = u[0]
        if not target_user and ctx.user:
            u = frappe.get_all(
                "User", filters={"telegram_username": ctx.user}, pluck="name", limit=1
            )
            if u:
                target_user = u[0]
    ...
    try:
        if _handle_photo_payload(ctx):
            try:
                metrics_inc(
                    "ferum_integration_telegram_webhook_total",
                    {"result": "success", "type": "photo"},
                )
            except Exception:
                pass
            return {"ok": True}
        ...
        try:
            metrics_inc(
                "ferum_integration_telegram_webhook_total", {"result": "error", "reason": "command"}
            )
        except Exception:
            pass
        ...
        try:
            if get_setting("telegram_alert_admin_on_failure"):
                ...
                from ferum_custom.ferum_custom.integrations.telegram import (  # type: ignore
    ...
    try:
        metrics_inc(
            "ferum_integration_telegram_webhook_total",
            {"result": "success", "type": "text" if ctx.text else "other"},
        )
    except Exception:
        pass
    return {"ok": True}
