...
def _resolve_telegram_chat_id(user: str) -> str | None:
    ...
    try:
        rows = frappe.get_all(
            "Telegram User Link", filters={"user": user}, pluck="chat_id", limit=1
        )
        if rows and rows[0]:
            return str(rows[0])
    except Exception:
...
def _channel_prefs(user: str) -> list[str]:
    ...
    try:
        vals = frappe.db.get_value(
            "User", user, ["notify_via_telegram", "notify_via_email"], as_dict=True
        )
    except Exception:
        vals = None
...
def notify(
    ...
    for user in users:
        ...
        for ch in channels:
            if ch == "email":
                delivered = (
                    _send_email(user, rendered.email_subject, rendered.email_body) or delivered
                )
                if delivered:
                    results["email"] += 1
            elif ch == "telegram":
