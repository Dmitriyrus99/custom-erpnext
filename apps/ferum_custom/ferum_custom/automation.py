...
def send_daily_overdue_report():
    ...
    frappe.log_info(
        f"Sent daily overdue report to {len(recipients)} recipients.", "Daily Overdue Report"
    )
...
def run_nightly_backup_to_gdrive():
    ...
    try:
        ...
        latest_backup_file = max(
            backup_files, key=lambda f: os.path.getmtime(os.path.join(backups_path, f))
        )
...
def run_permission_audit():
    ...
    if suspicious_page_perms:
        ...
        for perm in suspicious_page_perms:
            message_html += (
                f"<li>Роль <b>{perm.role}</b> имеет доступ к странице <b>{perm.page}</b></li>"
            )
        message_html += "</ul>"
    ...
    if suspicious_report_perms:
        ...
        for perm in suspicious_report_perms:
            message_html += (
                f"<li>Роль <b>{perm.role}</b> имеет доступ к отчету <b>{perm.report}</b></li>"
            )
        message_html += "</ul>"
    ...
    if not admin_recipients:
        frappe.log_error(
            "No System Manager found to send permission audit report.", "Permission Audit"
        )
        return
...
def get_report_recipients(roles: list[str] | tuple[str, ...] | set[str] | str) -> list[str]:
    ...
    for role in role_list:
        ...
        try:
            ...
            frappe.log_error(
                frappe.get_traceback(), f"get_report_recipients failed for role {role}"
            )
