...
from ferum_custom.telegram_bot.telegram_bot import main
...
def mock_frappe_client() -> FrappeClient:
    ...
    client.list_requests = AsyncMock(return_value=[
        {"name": "ISSUE-001", "title": "Test Issue 1", "status": "Open", "modified": "2023-01-01"},
        {"name": "ISSUE-002", "title": "Test Issue 2", "status": "In Progress", "modified": "2023-01-02"},
    ])
    client.update_request_status = AsyncMock(return_value={"name": "ISSUE-001", "status": "In Progress"})
...
async def setup_dispatcher_and_client(mock_frappe_client):
    ...
    with patch("ferum_custom.telegram_bot.telegram_bot.state.get_client", return_value=mock_frappe_client):
...
async def test_cmd_done(setup_dispatcher_and_client):
    ...
    message.answer.assert_called_once_with("ISSUE-001 — Завершена (In Progress)") # Note: mock returns 'In Progress'
...
async def test_on_photo_attach(setup_dispatcher_and_client):
    ...
    client.attach_to_request.assert_called_once_with("ISSUE-001", "file.jpg", b"fake_image_bytes", "image/jpeg")
