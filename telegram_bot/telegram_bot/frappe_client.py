...
class FrappeClient:
    ...
    async def _login(self) -> FrappeAuth:
        ...
        if isinstance(resp1, str) and resp1.lower() in {"logged in", "no app"}:
            ...
            return self._auth
        if isinstance(resp1, dict) and str(
            resp1.get("message") or resp1.get("status") or ""
        ).lower() in {
            "logged in",
            "no app",
        }:
    ...
    async def attach_to_request(
        self, name: str, file_name: str, content: bytes, content_type: str
    ) -> dict:
        """
